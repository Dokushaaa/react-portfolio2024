...
const ModalAddHero = ({ itemEdit }) => {
	...
	const mutation = useMutation({
		mutationFn: (values) =>
			queryData(
				itemEdit ? `/v1/hero/${itemEdit.hero_aid}` : `/v1/hero`,
				itemEdit ? "put" : "post",
				// `/v1/hero`,
				// `post`,
				values
	...
	const initVal = {
		hero_title: itemEdit ? itemEdit.hero_title : "",
		hero_image: itemEdit ? itemEdit.hero_image : "",
		hero_description: itemEdit ? itemEdit.hero_description : "",
		hero_context: itemEdit ? itemEdit.hero_context : "",
	};
	const yupSchema = Yup.object({
		hero_title: string().required("Headline Required*"),
		// hero_image: string().required("Image Required*"),
		hero_description: string().required("Description Required*"),
		hero_context: string().required("Context Required*"),
	});
	return (
		<>
			<ModalWrapper>
						...
						<Formik
							...
							onSubmit={async (values) => {
								...
								mutation.mutate({
									...values,
									hero_image:
										(itemEdit && itemEdit.hero_image === "") || photo
											? photo === null
												? itemEdit.hero_image
												: photo.name
											: values.hero_image,
							...
							<Form
								...
								<div className='grow overflow-y-auto'>
									<div className='input-wrap'>
										{photo || (itemEdit && itemEdit.hero_image !== "") ? (
											<img
												src={
													photo
														? URL.createObjectURL(photo) // preview
														: itemEdit.hero_image // check db
														? devBaseImgUrl + "/" + itemEdit.hero_image
														: null
												}
												alt='Photo'
