...
const ModalAddProglang = ({ itemEdit }) => {
	...
	const mutation = useMutation({
		mutationFn: (values) =>
			queryData(
				itemEdit ? `/v1/proglang/${itemEdit.proglang_aid}` : `/v1/proglang`,
				itemEdit ? "put" : "post",
				// `/v1/proglang`,
				// `post`,
				values
		...
		onSuccess: (data) => {
			queryClient.invalidateQueries({ queryKey: ["proglang"] });
			if (data.success) {
	...
	const initVal = {
		proglang_title: itemEdit ? itemEdit.proglang_title : "",
		proglang_image: itemEdit ? itemEdit.proglang_image : "",
		proglang_description: itemEdit ? itemEdit.proglang_description : "",
	};
	const yupSchema = Yup.object({
		proglang_title: string().required("Headline Required*"),
		// proglang_image: string().required("Image Required*"),
		proglang_description: string().required("Description Required*"),
	});
	return (
		<>
			<ModalWrapper>
						...
						<Formik
							...
							onSubmit={async (values) => {
								...
								mutation.mutate({
									...values,
									proglang_image:
										(itemEdit && itemEdit.proglang_image === "") || photo
											? photo === null
												? itemEdit.proglang_image
												: photo.name
											: values.proglang_image,
								});
							}}>
							<Form
								action=''
								...
								<div className='grow overflow-y-auto'>
									<div className='input-wrap'>
										{photo || (itemEdit && itemEdit.proglang_image !== "") ? (
											<img
												src={
													photo
														? URL.createObjectURL(photo) // preview
														: itemEdit.proglang_image // check db
														? devBaseImgUrl + "/" + itemEdit.proglang_image
														: null
												}
												alt='Photo'
										...
										{(photo !== null ||
											(itemEdit && itemEdit.proglang_image !== "")) && (
...
export default ModalAddProglang;
